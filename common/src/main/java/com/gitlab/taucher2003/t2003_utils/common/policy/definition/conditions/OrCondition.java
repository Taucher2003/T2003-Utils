...
public class OrCondition<RESOURCE, CONTEXT, ABILITY> implements Condition<RESOURCE, CONTEXT, ABILITY> {

    private final List<Condition<RESOURCE, CONTEXT, ABILITY>> conditions;

    public OrCondition(List<Condition<RESOURCE, CONTEXT, ABILITY>> conditions) {
    ...
    public Condition<RESOURCE, CONTEXT, ABILITY> simplify() {
        var simplifiedConditions = conditions().flatMap(condition -> {
            ...
            if (OrCondition.class.equals(simplified.getClass())) {
                return ((OrCondition<RESOURCE, CONTEXT, ABILITY>) simplified).conditions();
            }
            return Stream.of(simplified);
        }).collect(Collectors.toList());

        return new OrCondition<>(simplifiedConditions);
    }
